    ...
    try {
      ...
      const promises = validations.map(async (validation) => {
        try {
          ...
          const errors = await validation.validate(this);
          report.push({ validation, isValid: errors.length ? false : true });
          return finishValidation(validation, errors);
        } catch(e) {
          console.error(`Error while executing validation ${validation.name}: ${e}`);
          report.push({ validation, isValid: false });
          return finishValidation(validation, null, false);
        }
      });
